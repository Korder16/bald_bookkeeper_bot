...
class bald_bookeeper_bot_db_client:
    ...
    def get_username_by_tg_id(self, id: int) -> str:
        sql_query = f'select u.name from usernames u, telegram_accounts t where t.telegram_id = {id} and t.id = u.id'
    ...
    def get_username_by_dota_id(self, id: int) -> str:
        sql_query = f'select u.name from dota_accounts d, usernames u where d.dota_account_id = {id} and d.id = u.id'
